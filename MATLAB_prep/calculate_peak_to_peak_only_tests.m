function [peak_to_peak_amplitudes, signal_noise_peak_to_peak_amplitude, noise_rms] = calculate_peak_to_peak_only_tests( ...
    ...
    for i = 1:number_of_tests
        ...
        [ch3_no_artifacts] = ArtifactRemovalFun(time_ms, ch1, ch3, pulse_width);
        %[ch3_no_artifacts] = ArtifactRemovalFun(ch1, ch3, pulse_width);
        ...
        if save_csv == 1
            % Create directory if needed (only once)
            if i == 1 && ~exist(output_dir, 'dir'), mkdir(output_dir); end
            
            % Combine time and processed channel
            %processed_data_to_save = [time_ms, ch3];
            processed_data_to_save = [time_ms, ch3, ch1];
            ...
            % Write CSV
            writematrix(processed_data_to_save, output_path);
